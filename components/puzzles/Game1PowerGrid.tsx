...
export default function Game1PowerGrid({ onComplete }: Game1PowerGridProps) {
    ...
    // Hardcoded logic: Top ON, Middle ON, Bottom OFF. Zone 1: AND, Zone 2: XOR
    ...
    const handleDrop = (zone: 1 | 2, gate: string) => {
...
const Switch = ({ on, toggle, label }: { on: boolean; toggle: () => void; label: string }) => (
...
const DropZone = ({ value, onDrop, onClear, label }: { value: string | null; onDrop: (gate: string) => void; onClear: () => void; label: string }) => (
...
const OutputNode = ({ success, error }: { success: boolean; error: boolean }) => (
