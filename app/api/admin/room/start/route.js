...
function dealPuzzles(allPuzzleIds, teams, puzzlesPerTeam) {
    ...
    for (const tid of teams) {
        ...
        assignments[tid] = shuffle(teamPuzzles);
    }
    return assignments;
}
...
export async function POST(req) {
    try {
        ...
        const assignments = dealPuzzles(allPuzzleIds, room.teamTids, room.puzzlesPerTeam);
        ...
        // Update each team
        for (const tid of room.teamTids) {
            await Team.findOneAndUpdate(
                { tid },
                {
                    status: 'playing',
                    assignedPuzzleIds: assignments[tid],
                    currentIndex: 0,
                    solvedPuzzleIds: [],
                    penaltySeconds: 0,
